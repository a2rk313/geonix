export image_name := env("IMAGE_NAME", "geonix-gnome")
...
_run-vm $target_image $tag $type $config:
    ...
    if [[ $type == anaconda-iso ]]; then
        ...
        boot_target="/boot.iso"
        build_recipe=$([[ "$target_image" == *plasma* ]] && echo build-iso-kde || echo build-iso-gnome)
    else
        image_file="output/${type}/disk.${type}"
        boot_target="/boot.${type}"
        build_recipe="build-${type}"
    fi

    if [[ ! -f "${image_file}" ]]; then
        just "${build_recipe}" "$target_image" "$tag"
    fi
    ...
    run_args+=(--volume "${PWD}/${image_file}":"${boot_target}")
...
import "build_files/60-geonix.just"
